...
SELECT * FROM EMPLOYEES;
SELECT FIRST_NAME || LAST_NAME AS 이름, PHONE_NUMBER AS 연락처 FROM EMPLOYEES
...
SELECT FIRST_NAME || LAST_NAME AS 이름, PHONE_NUMBER AS 연락처, HIRE_DATE AS 고용날짜 FROM EMPLOYEES
...
SELECT FIRST_NAME || LAST_NAME AS 이름, SALARY AS 급여, PHONE_NUMBER AS 연락처 FROM EMPLOYEES
	WHERE MANAGER_ID = '120'


--요구사항.011.employees
--특정 부서(60, 80, 100)에 속한 직원들의 이름, 연락처, 이메일, 부서ID 가져오시오.
SELECT FIRST_NAME || LAST_NAME AS 이름, PHONE_NUMBER AS 연락처, EMAIL AS 이메일, JOB_ID AS 부서ID FROM EMPLOYEES
...
SELECT * FROM TBLINSA;
SELECT name AS 이름, JIKWI AS 직위, TEL AS 전화번호 FROM TBLINSA
