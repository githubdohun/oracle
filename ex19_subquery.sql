...
/*
   ...
   2. Sub Query, 서브 쿼리, 부속 질의
      - 하나의 문장안에(SELECT, INSERT, UPDATE, DELETE) 또 다른 문장(SELETE)이 들어있는 쿼리
      - 하나의 SELECT안에 또 다른 SELECT문이 들어있는 쿼리
      - 삽입위치 > SELECT절, FROM절, WHERE절, GROUP BY절, HAVING절, ORDER BY절
...
--tblmen, tblwomen 남자 키가 166cm인 여자친구의 키?
SELECT * FROM tblmen;
...
/*
   ...
   1. 조건절 > 비교값으로 사용
      ...
      c. 반환값이 1행 N열 > 다중값 반환 > 그룹비교 > N컬럼:N컬럼 (***)
...
--급여가 260만원 이상 받은 직원과 같은부서, 같은 지역 > 직원 명단
SELECT 
   * 
FROM tblinsa
...
/*
   ...
   2. 컬럼리스트 > 출력값으로 사용
      - 반드시 결과값이 1행 1열이어야 한다(스칼라 쿼리라고 칭하며 원자값을 반환함)
...
SELECT 
   name, buseo, basicpay,
   (SELECT round(avg(basicpay)) FROM tblinsa b WHERE b.buseo = a.buseo) AS 평균급여
FROM tblinsa a;


--남자(이름, 키, 몸무게)와 여자(이름, 키, 몸무게)
SELECT * FROM tblmen;
SELECT * FROM tblwomen;
