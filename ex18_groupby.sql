...
-- 부서의 과장/부장(WHERE) 인원 수가 3명이 넘는(HAVING) 결과
SELECT
	buseo,
	count(*)
FROM tblinsa
...
-- job_id 그륩 > 통계		
SELECT
job_id, count(*) AS 인원수, round(avg(salary)) AS 평균급여
FROM employees
	GROUP BY job_id;
...
SELECT
	buseo AS 부서명,
	count(*) AS 총인원,
	count(decode(jikwi, '부장', 1)) AS "그룹별 부장 수",
	count(decode(jikwi, '과장', 1)) AS "그룹별 과장 수",
	count(decode(jikwi, '대리', 1)) AS "그룹별 대리 수",
	count(decode(jikwi, '사원', 1)) AS "그룹별 사원 수"
FROM tblinsa
	GROUP BY buseo;
...
-- rollup > 다중 그룹 컴럼 > 수직관계 (전체 인원 총계, 부서별 총계)
